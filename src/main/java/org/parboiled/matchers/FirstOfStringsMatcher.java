...
public class FirstOfStringsMatcher extends FirstOfMatcher {
    ...
    static class Record {
        ...
        private Record(final char[] chars, final Record[] subs, final boolean complete) {
            this.chars = chars;
            this.subs = subs;
    ...
    static Record createRecord(final int pos, final char[][] strings) {
        final Map<Character, Set<char[]>> map = new TreeMap<Character, Set<char[]>>();
    ...
    private static void verify(final char[][] strings) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            for (int j = i + 1; j < length; j++) {
                ...
                final String msg = a.length == b.length ? sa + " is specified twice in a FirstOf(String...)" : sa +
